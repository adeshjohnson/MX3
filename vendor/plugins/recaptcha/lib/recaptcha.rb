  ...
  module ReCaptcha
    ...
    module Helper
      ...
      def recaptcha_tags(options = {})
        # Default options
        key   = options[:public_key] ||= ReKey.get_public()
    ...
    module Controller
      ...
      def verify_recaptcha(model = nil)
        ...
        raise ReCaptchaError, "No private key specified." unless ReKey.get_private()
        begin
          recaptcha = Net::HTTP.post_form URI.parse("http://#{RECAPTCHA_VERIFY_SERVER}/verify"), {
            :privatekey => ReKey.get_private(),
        ...
        model.errors.add_to_base "Oops, we failed to validate your word verification response. Please try again." if model
        return false
      rescue Exception => e
