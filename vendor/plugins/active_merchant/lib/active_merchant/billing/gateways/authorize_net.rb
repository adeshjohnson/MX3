  ...
  module Billing #:nodoc:
    ...
    class AuthorizeNetGateway < Gateway
      ...
      def add_customer_data(post, options)
        if options["gateways"] and options["gateways"]["authorize_net"]["billing_address"]
          post[:email] = options["gateways"]["authorize_net"]["billing_address"]["email"].to_s
      ...
      def message_from(results)
        ...
        MorLog.my_debug("authorize_net Code : #{results[:response_code]} => #{results[:response_reason_code]} | #{results[:response_reason_text]}, AVS => #{results[:avs_result_code]}, CVV => #{results[:card_code]}", 1)

        out = results[:response_reason_text].nil? ? '' : results[:response_reason_text][0..-2]
        if results[:response_reason_text].to_s.include?('This transaction cannot be accepted.')
          out += " Please check your Authorize.Net settings."
        end
