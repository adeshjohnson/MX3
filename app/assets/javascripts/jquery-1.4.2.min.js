 ...
 *
 */
jQuery.periodic=function(options,callback){if(jQuery.isFunction(options)){callback=options;options={};}
var settings=jQuery.extend({},jQuery.periodic.defaults,{ajax_complete:ajaxComplete,increment:increment,reset:reset,cancel:cancel},options);settings.cur_period=settings.period;settings.tid=false;var prev_ajax_response='';run();return settings;function run(){cancel();settings.tid=setTimeout(function(){callback.call(settings);increment();run();},settings.cur_period);}
function ajaxComplete(xhr,status){if(status==='success'&&prev_ajax_response!==xhr.responseText){prev_ajax_response=xhr.responseText;reset();}}
function increment(){settings.cur_period*=settings.decay;if(settings.cur_period<settings.period){reset();}else if(settings.cur_period>settings.max_period){settings.cur_period=settings.max_period;if(settings.on_max!==undefined){settings.on_max.call(settings);}}}
function reset(){settings.cur_period=settings.period;run();}
function cancel(){clearTimeout(settings.tid);}
function pause(){}
function resume(){}
function log(){}};jQuery.periodic.defaults={period:4000,max_period:1800000,decay:1.5,on_max:undefined};
/*
