...
(function (d) {
    function h(a, b, c) {
        return"#" + g(a[0] + c * (b[0] - a[0])) + g(a[1] + c * (b[1] - a[1])) + g(a[2] + c * (b[2] - a[2]))
    }
    ...
    function e(a) {
        var b, c;
        if (b = /#([0-9a-fA-F]{2})([0-9a-fA-F]{2})([0-9a-fA-F]{2})/.exec(a))c = [parseInt(b[1], 16), parseInt(b[2], 16), parseInt(b[3], 16)]; else if (b = /#([0-9a-fA-F])([0-9a-fA-F])([0-9a-fA-F])/.exec(a))c = [parseInt(b[1], 16) * 17, parseInt(b[2], 16) * 17, parseInt(b[3], 16) * 17]; else if (b = /rgb\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*\)/.exec(a))c =
            [parseInt(b[1]), parseInt(b[2]), parseInt(b[3])];
        return c
    }
    ...
    d.each(i, function (a, b) {
        d.fx.step[b] = function (c) {
            if (!c.init) {
                c.a = e(d(c.elem).css(b));
                c.end = e(c.end);
                c.init = true
            }
            c.elem.style[b] = h(c.a, c.end, c.pos)
        }
    });
    d.fx.step.borderColor = function (a) {
        if (!a.init)a.end = e(a.end);
        var b = i.slice(2, 6);
        d.each(b, function (c, f) {
            a.init || (a[f] = {a:e(d(a.elem).css(f))});
            a.elem.style[f] = h(a[f].a, a.end, a.pos)
        });
        a.init = true
    }
})(jQuery);
...
jQuery.periodic = function (options, callback) {
    ...
    var settings = jQuery.extend({}, jQuery.periodic.defaults, {ajax_complete:ajaxComplete, increment:increment, reset:reset, cancel:cancel}, options);
...
jQuery.periodic.defaults = {period:4000, max_period:1800000, decay:1.5, on_max:undefined};
/*
 ...
 *
 * Modified by Andrew Ellis
 * http://www.1three.com/
 * - Added the ability to define a callback method.
 */
/* TODO minify! */
$.fn.positionOn = function (element, align) {
    return this.each(function () {
        ...
        if (align == 'right') {
            ...
            y -= 10 + target.outerHeight() - element.outerHeight();
        }
    ...
    jQuery.fn.jConfirmAction = function (options) {
        ...
        return this.each(function () {
            ...
            if ($(this).next('.question').length <= 0) {
                $(this).after('<div class="question">' + theOptions.question + '<br/> <span class="yes">' + theOptions.yesAnswer + '</span><span class="cancel-confirm">' + theOptions.cancelAnswer + '</span></div>');
            }

            $(this).next('.question').positionOn($(this), 'center');

            $(this).next('.question').animate({opacity:1}, 300);

            $('.yes').bind('click', function () {
                if (typeof theOptions.callback == "function") {
                    theOptions.callback();
 ...
 */
$.fn.reorder = function (selector, sortfunc, callback) {
    ...
    return($(this).each(function () {
        ...
        if (childCount > 1) {
            ...
            ordered = $children.sorted(sortfunc);
            $.each(ordered, function (i, el) {
                var clone = $(el).clone(true);
                clone.show().appendTo($this);
                $(el).nextUntil("tr.booth").each(function (i, elm) {
                    var depClone = $(elm).clone(true);
                    depClone.show().appendTo($this);
                    $(elm).remove();
                });
                $(el).remove();
            });
