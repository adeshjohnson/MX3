class Did < ActiveRecord::Base
  ...
  def did_prov_rates
    did_check_rates('provider')
  end

  def did_incoming_rates
    did_check_rates('incoming')
  end

  def did_owner_rates
    did_check_rates('owner')
  end

  def did_check_rates(type)
    dr = Didrate.find(:all, :conditions => ["did_id = ? AND rate_type = ?", self.id, type])
    if dr.size == 0
      dr = Didrate.new(:did_id => self.id, :rate_type => type.to_s)
      dr.save ? dr = [dr] : dr = []
    end
    dr
  end
