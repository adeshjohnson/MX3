class Iplocation < ActiveRecord::Base
  ...
  def Iplocation::get_location_from_whatismyipaddress(ip, loc)
    if check_ip(ip) == true
      loc.ip = ip
      begin
        response = Net::HTTP.get_response("whatismyipaddress.com", "/ip/#{ip}")
        res = response.body
  ...
  def Iplocation::get_location_from_ip_address(ip, loc)
    if check_ip(ip) == true
      loc.ip = ip
      begin        
        res = Net::HTTP.get_response("www.ip-adress.com", "/ip_tracer/#{ip}").body
        if res and res.length > 0
  ...
  def Iplocation::get_location_from_google_geo(dst, dir, loc, prefix)
    ...
    begin 
      if dst != ""  
        address = dir+" "+dst
        res = Net::HTTP.get_response(URI.parse("http://maps.google.com/maps/geo?q=#{address.gsub(" ", "+")}&output=csv&key=#{ApiKey.get}")).body.to_s.split(",")
      end

      if dst == "" or res[0] != "200"  
        res = Net::HTTP.get_response(URI.parse("http://maps.google.com/maps/geo?q=#{dir.gsub(" ", "+")}&output=csv&key=#{ApiKey.get}")).body.split(",")
        dst = ""
      end
  ...
  def Iplocation::get_location(ip, prefix = nil)    
    loc = Iplocation.find(:first, :conditions => ["ip = ?", ip])
    return loc if loc 
    loc = Iplocation.new()
    if ip.to_s == ""
      loc.latitude = 0
    ...
    if prefix == nil 
      ...
      dst = Destination.find(:first, :conditions => ["prefix = ?", ip])
      if dst
        direction = dst.direction
        direction ? dir = direction.name.to_s : dir = ""
        dst = dst.name.to_s
        loc = get_location_from_google_geo(dst, dir, loc, prefix)
      end
    end
    loc.save
    return loc   
