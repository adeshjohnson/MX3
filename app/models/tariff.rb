class Tariff < ActiveRecord::Base
  ...
  validates_uniqueness_of :name, :scope => :purpose
...
=end
  ...
  def add_new_rate(dest_id, rate_value, increment, min_time)
    rate = Rate.new
...
WHERE rates.tariff_id = #{self.id} AND tmp_dest_groups.rate = ratedetails.rate
  ...
  def check_types_periods(options={})
    ...
    typ = ["wd", "fd"].include?(options[:rate_day_type].to_s) ?  " AND daytype = '#{options[:rate_day_type].to_s}'" : ''
    ratesd = Ratedetail.find(:all, :joins => "LEFT JOIN rates ON (ratedetails.rate_id = rates.id)", :conditions => ["rates.tariff_id = ? AND ((? < start_time AND ? > end_time) OR (? BETWEEN start_time AND end_time) OR (? BETWEEN start_time AND end_time)) #{typ} AND ( start_time != ? OR end_time != ? )", id, a1,a2,a1,a2,a1,a2 ])
    notice_2 = ''
    # checking time periods for collisions
    if ratesd and ratesd.size.to_i > 0
      notice_2 = _('Tarrif_import_incorect_time')
      notice_2 += '<br /> * ' + _('Please_select_period_without_collisions')
  ...
  def analize_file(name, options)
    ...
    arr[:destinations_to_update] = ActiveRecord::Base.connection.select_value("SELECT COUNT(*) AS d_all FROM #{name} WHERE ned_update = 1").to_i if options[:imp_update_dest_names].to_i == 1 and options[:imp_dst] >= 0
  ...
  def update_rates_from_csv(name, options, options2)
    ...
    sql = "UPDATE ratedetails, (SELECT rates.id AS nrate_id, #{name}.* FROM rates join destinations ON (destinations.id = rates.destination_id) JOIN #{name} ON (replace(col_#{options[:imp_prefix]}, '\\r', '') = destinations.prefix) where rates.tariff_id = #{id} AND f_error = 0 AND not_found_in_db = 0) AS temp
            SET ratedetails.rate = replace(replace(col_#{options[:imp_rate]}, '\\r', ''), '#{options[:dec]}', '.'),
  ...
  def create_rates_from_csv(name,options, options2)
    ...
    sql="INSERT INTO rates (tariff_id, destination_id)
    SELECT #{id}, destinations.id FROM #{name}
    join destinations on (replace(col_#{options[:imp_prefix]}, '\\r', '') = destinations.prefix)
    LEFT join rates on (destinations.id = rates.destination_id and rates.tariff_id = #{id})
    WHERE rates.id IS NULL AND f_error = 0"
    ActiveRecord::Base.connection.update(sql)
  ...
  def insert_ratedetails(name, options, options2)
    ...
    #    bad_prefix_sql = options2[:bad_destinations].to_i > 0 ? "AND replace(col_#{options[:imp_prefix]}, '\\r', '') NOT IN (#{options2[:bad_prefixes].join(',')})" : ''
    s = [] ; ss=[]
    ["rate", "increment_s", "min_time", "connection_fee", "daytype"].each{ |col|
      case col
      when "daytype"
        ["wd", "fd"].include?(options[:imp_date_day_type].to_s) ? day_type = options[:imp_date_day_type].upcase : day_type = ""
      ...
      if country_code.blank? or options[:imp_cc] == -1
        ...
        if pfound == 1
          ...
          if options[:imp_subcode].to_i < 0
            ...
            ActiveRecord::Base.connection.execute("UPDATE #{name} SET short_prefix = '#{short_prefix}' WHERE id = #{row['i_id']}")
          end
        else
          bad << row['i_id']
        end
