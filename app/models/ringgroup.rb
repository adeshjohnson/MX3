class Ringgroup < ActiveRecord::Base
  ...
  def devices
    Device.find(:all, :include=>[:ringgroups_devices], :conditions=>['ringgroup_id=?', id] , :order=>'priority ASC')
  end

  def free_devices(dev_id = -1)
    if dev_id.to_i > 0
      Device.find(:all, :select=>'devices.*' , :conditions=>['id NOT IN (SELECT device_id FROM ringgroups_devices WHERE ringgroup_id = ?) AND user_id = ?', id, dev_id])
    else
      Device.find(:all, :select=>'devices.*' , :conditions=>['id NOT IN (SELECT device_id FROM ringgroups_devices WHERE ringgroup_id = ?)', id])
    end
  end
  ...
  def update_exline(ext = -1)
    ...
    Extline.destroy_all ["exten = ? AND app = ?", exten, 'Dial']
    Extline.destroy_all ["exten = ? AND app = ?", exten, 'Goto']
    Extline.destroy_all ["exten = ? AND app = ?", exten, 'Set']
    i = 1
    if !cid_prefix.blank?
  ...
  def  Ringgroup.ringgroups_order_by(params, options)
    ...
    when "id" :     order_by = " ringgroups.id "
    when "name" :   order_by = " dialplans.name "
    when "extension" :  order_by = " dialplans.data2 "
    when "comment" :  order_by = " ringgroups.comment "
    when "ring_time" :  order_by = " ringgroups.timeout "
    when "options" :  order_by = " ringgroups.options "
    when "strategy" :  order_by = " ringgroups.strategy "
    when "prefix" :  order_by = " ringgroups.cid_prefix "
