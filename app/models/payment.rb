...
class Payment < ActiveRecord::Base
  ...
  def Payment.add_for_card(card, amount)
    Payment.add_global({:paymenttype=>'Card', :tax => card.cardgroup.get_tax.count_tax_amount(amount), :currency => card.cardgroup.tell_balance_in_currency, :user_id=>card.id, :card=>1, :amount=>amount, :owner_id => card.cardgroup.owner_id})
  end
