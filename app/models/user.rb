class User < ActiveRecord::Base
  ...
  cattr_accessor :sistem_time_ofset

  has_many :devices, :conditions => "accountcode != 0"
  has_many :actions
  belongs_to :lcr
  belongs_to :tariff
  belongs_to :sms_lcr
  ...
  def fax_devices
    Device.find(:all, :conditions => "user_id = #{self.id} AND device_type = 'FAX'")
  end
...
=end
  ...
  def own_outgoing_calls_stats_in_period(period_start, period_end, calldate_index = 0)
    ...
    show_zero_calls = self.invoice_zero_calls.to_i
    ...
    val = ActiveRecord::Base.connection.select_all("SELECT count(calls.id) as calls, SUM(calls.user_price + calls.did_price + calls.did_inc_price) as price FROM calls JOIN devices ON (calls.src_device_id = devices.id) WHERE calls.card_id = 0 AND disposition = 'ANSWERED' and devices.user_id = #{self.id} AND calldate BETWEEN '#{period_start}' AND '#{period_end}' #{zero_calls_sql};")
    val2 = ActiveRecord::Base.connection.select_all("SELECT count(calls.id) as calls, SUM(calls.user_price + calls.did_price + calls.did_inc_price) as price FROM calls JOIN devices ON (calls.dst_device_id = devices.id) WHERE calls.card_id = 0 AND disposition = 'ANSWERED' and devices.user_id = #{self.id} AND calldate BETWEEN '#{period_start}' AND '#{period_end}' #{zero_calls_sql};")
    MorLog.my_debug("SELECT count(calls.id) as calls, SUM(calls.user_price + calls.did_price + calls.did_inc_price) as price FROM calls JOIN devices ON (calls.src_device_id = devices.id) WHERE calls.card_id = 0 AND disposition = 'ANSWERED' and devices.user_id = #{self.id} AND calldate BETWEEN '#{period_start}' AND '#{period_end}' #{zero_calls_sql};", 1)
    MorLog.my_debug("SELECT count(calls.id) as calls, SUM(calls.user_price + calls.did_price + calls.did_inc_price) as price FROM calls JOIN devices ON (calls.dst_device_id = devices.id) WHERE calls.card_id = 0 AND disposition = 'ANSWERED' and devices.user_id = #{self.id} AND calldate BETWEEN '#{period_start}' AND '#{period_end}' #{zero_calls_sql};", 1)
  ...
  def users_outgoing_calls_stats_in_period(period_start, period_end, calldate_index = 0)
    ...
    index_cond = calldate_index == 1 ? ' USE INDEX (calldate) ' : ''
    # sql = "SELECT count(calls.id) as calls, #{SqlExport.replace_price('SUM(calls.reseller_price + calls.did_price + calls.did_inc_price)', 'price')} FROM calls #{index_cond} WHERE calls.card_id = 0 AND disposition = 'ANSWERED' and calls.reseller_id = '#{self.id}' AND calldate BETWEEN '#{period_start}' AND '#{period_end}' #{zero_calls_sql}"
    sql = "SELECT count(calls.id) as calls, SUM(calls.reseller_price + calls.did_price + calls.did_inc_price) as price
           FROM calls #{index_cond}
           WHERE calls.card_id = 0 AND disposition = 'ANSWERED'
           and calls.reseller_id = '#{self.id}'
  ...
  def incoming_received_calls_stats_in_period(period_start, period_end, calldate_index = 0)
    ...
    index_cond = calldate_index == 1 ? ' USE INDEX (calldate) ' : ''
    sql = "SELECT count(calls.id) as calls, #{SqlExport.replace_price('SUM(calls.user_price + calls.did_price + calls.did_inc_price)', {:reference=> 'price'})}
                  FROM calls #{index_cond}
                  JOIN devices ON (calls.dst_device_id = devices.id)
                  WHERE calls.card_id = 0 AND disposition = 'ANSWERED' AND calldate BETWEEN '#{period_start}' AND '#{period_end}' AND devices.user_id = #{self.id} AND calls.did_price > 0;"
  ...
  def incoming_made_calls_stats_in_period(period_start, period_end, calldate_index = 0)
    ...
    sql = "SELECT count(calls.id) as calls, #{SqlExport.replace_price('SUM(calls.user_price + calls.did_price + calls.did_inc_price)', {:reference=> 'price'})}
                  ...
                  WHERE calls.card_id = 0 AND disposition = 'ANSWERED' AND calldate BETWEEN '#{period_start}' AND '#{period_end}' AND devices.user_id = #{self.id} AND calls.did_inc_price > 0;"
  ...
  def create_default_device(options={})
    ...
    if (set_location_id.blank? or set_location_id.to_i == 1) and owner and owner.usertype == 'reseller'
    ...
    if device.save
      ...
      device.create_vm(device.extension, pass, self.first_name + " " + self.last_name, email)
      device.update_cid(Confline.get_value("Default_device_cid_name", owner_id), Confline.get_value("Default_device_cid_number", owner_id))
...
=end
  ...
  def pay_subscriptions(year, month)
    ...
    self.balance -= b
    if user_type == "postpaid" and (self.balance + self.credit < 0) and self.credit != -1
      changed = 1
  ...
  def reseller_allow_providers_tariff?
    usertype == "reseller"  and own_providers == 1
  end

  def is_allow_manage_providers?
    usertype == "admin" or self.reseller_allow_providers_tariff?
  end
  ...
  def load_lcrs(*arr)
    if usertype == 'accountant'
      ...
      if !own_providers? and usertype == 'reseller'
        if arr[1] and arr[1].include?(:conditions)
...
GROUP BY terminators.id;").map{|t| t.id }
  ...
  def User.users_order_by(params, options)
    ...
    when "acc" then order_by = "users.id"
    when "nice_user" then order_by = "nice_user"
    when "user" then order_by = "nice_user"
    when "username" then order_by = "users.username"
    when "usertype" then order_by = "users.usertype"
    when "balance" then order_by = "users.balance"
    when "account_type" then order_by = "users.postpaid"
    else
  ...
  def load_tariffs
    usertype == "accountant" ? owner = 0 : owner = id
  ...
  def User.create_from_registration(params, owner, reg_ip, free_ext, pin, pasw, nan, api=0)
    ...
    user.credit = 0 if user.postpaid == 0
    if user.owner_id != 0
  ...
  def update_from_edit(params, current_user, tax_from_params, monitoring_a, rec_a, api = 0)
    ...
    if self.usertype == "reseller" and self.own_providers.to_i == 0

      Action.add_action_hash(current_user.id, {:action => 'reseller_lcr_change', :target_id => self.id, :target_type => "user", :data => user_old.lcr_id, :data2 => self.lcr_id})


      User.find(:all, :conditions => ["owner_id = ?", self.id]).each { |res_user|
        res_user.lcr_id = self.lcr_id
        res_user.save
      }

      Cardgroup.find(:all, :conditions => ["owner_id = ?", self.id]).each { |cg|
        cg.lcr_id =self.lcr_id
        cg.save
      }
    end
  ...
  def warning_email_hour
    ...
    if b != -1
      c = b.to_i + time_zone.to_i - User.sistem_time_ofset.to_i
      b = c.to_i > 24 ? c - 24 : c
      b = c.to_i < 0 ? c + 24 : b
    else
      b
  ...
  def warning_email_hour=value
    if value != -1
      b = value.to_i - time_zone.to_i + User.sistem_time_ofset.to_i
      c = b.to_i > 24 ? b - 24 : b
      c = b.to_i < 0 ? b + 24 : c
    else
      c = value
  ...
  def validate_from_update(current_user, params, allow_edit, api = 0)
    notice = ''
    co = current_user.usertype == 'accountant' ? 0 : current_user.id
    if current_user.usertype == 'accountant' and !allow_edit
      notice = _('You_have_no_editing_permission')
    end

    if current_user.usertype == "reseller" and !params[:user][:usertype].blank? and params[:user][:usertype].to_s != "user"
      notice = _('Dont_be_so_smart')
    end

    if current_user.usertype == "accountant" and !params[:user][:usertype].blank? and params[:user][:usertype] == "admin"
      notice = _('Dont_be_so_smart')
    end
  ...
  def user_time(time)
    time + time_zone.hour - User.sistem_time_ofset.to_i.hour
  end

  def system_time(time, only_date = 0)
    t = time.class == 'Time' ? time : time.to_time
    if only_date == 0
      (t - time_zone.hour + User.sistem_time_ofset.to_i.hour).to_s(:db)
    else
      (t - time_zone.hour + User.sistem_time_ofset.to_i.hour).to_date.to_s(:db)
    end
  end
  ...
  def User.get_zones
    ...
    m = [
   		["(GMT-11:00) International Date Line West, Midway Island, Samoa"	,	"International Date Line West", -11],
			#["(GMT-11:00) Midway Island"	,	"Midway Island", -11],
			#["(GMT-11:00) Samoa"	,	"Samoa", -11],
			["(GMT-10:00) Hawaii"	,	"Hawaii", -10],
			["(GMT-09:00) Alaska"	,	"Alaska", -9],
			["(GMT-08:00) Pacific Time (US & Canada), Tijuana"	,	"Pacific Time (US & Canada)", -8],
			#["(GMT-08:00) Tijuana"	,	"Tijuana", -8],
			["(GMT-07:00) Arizona, Chihuahua, Mazatlan, Mountain Time (US & Canada)"	,	"Arizona", -7],
			#["(GMT-07:00) Chihuahua"	,	"Chihuahua", -7],
			#["(GMT-07:00) Mazatlan"	,	"Mazatlan", -7],
			#["(GMT-07:00) Mountain Time (US & Canada)"	,	"Mountain Time (US & Canada)", -7],
			["(GMT-06:00) Central Time (US & Canada), Guadalajara, Mexico City, Saskatchewan"	,	"Central America", -6],
			...
			["(GMT-05:00) Bogota, Eastern Time (US & Canada), Indiana (East), Lima, Quito"	,	"Bogota", -5],
			...
			["(GMT-04:00) Atlantic Time (Canada), Georgetown, La Paz, Santiago"	,	"Atlantic Time (Canada)", -4],
      ...
      #			["(GMT-04:00) Santiago"	,	"Santiago", -4],
			...
			["(GMT-03:00) Brasilia, Buenos Aires, Greenland"	,	"Brasilia", -3],
      #			["(GMT-03:00) Buenos Aires"	,	"Buenos Aires", -3],
      #			["(GMT-03:00) Greenland"	,	"Greenland", -3],
			["(GMT-02:00) Mid-Atlantic"	,	"Mid-Atlantic", -2],
			["(GMT-01:00) Azores, Cape Verde Is"	,	"Azores", -1],
      #			["(GMT-01:00) Cape Verde Is."	,	"Cape Verde Is.", -1],
			["(GMT+00:00) Casablanca, Dublin, Edinburgh, Lisbon, London, Monrovia"	,	"Casablanca", 0],
      ...
      #			["(GMT+00:00) Monrovia"	,	"Monrovia", 0],
			#["(GMT+00:00) UTC"	,	"UTC", 0],
			["(GMT+01:00) Amsterdam, Belgrade, Berlin, Madrid, Paris, Prage,  Rome "	,	"Amsterdam", 1],
      ...
      #			["(GMT+01:00) Zagreb"	,	"Zagreb",1],
			["(GMT+02:00) Athens, Cairo, Helsinki, Istanbul, Kyiv, Minsk, Riga, Tallinn, Vilnius"	,	"Athens",2],
      ...
      #			["(GMT+02:00) Vilnius"	,	"Vilnius",2],
			["(GMT+03:00) Baghdad, Kuwait, Nairobi, Riyadh"	,	"Baghdad",3],
      #			["(GMT+03:00) Kuwait"	,	"Kuwait",3],
      #			["(GMT+03:00) Nairobi"	,	"Nairobi",3],
      #			["(GMT+03:00) Riyadh"	,	"Riyadh",3],
			#["(GMT+03:30) Tehran"	,	"Tehran",3.5],
			["(GMT+04:00) Abu Dhabi, Baku, Moscow, Muscat, Tbilisi, Volgograd, Yerevan"	,	"Abu Dhabi",4],
      ...
      #			["(GMT+04:00) Yerevan"	,	"Yerevan",4],
			...
			["(GMT+05:00) Islamabad, Karachi, Tashkent"	,	"Islamabad",5],
      #			["(GMT+05:00) Karachi"	,	"Karachi",5],
      #			["(GMT+05:00) Tashkent"	,	"Tashkent",5],
			#["(GMT+05:30) Chennai"	,	"Chennai",5.5],
			...
			#["(GMT+05:45) Kathmandu"	,	"Kathmandu",5.6],
			["(GMT+06:00) Almaty, Astana, Dhaka, Ekaterinburg"	,	"Almaty",6],
      #			["(GMT+06:00) Astana"	,	"Astana",6],
      #			["(GMT+06:00) Dhaka"	,	"Dhaka",6],
      #			["(GMT+06:00) Ekaterinburg"	,	"Ekaterinburg",6],
			#["(GMT+06:30) Rangoon"	,	"Rangoon",6.5],
			["(GMT+07:00) Bangkok, Hanoi, Jakarta, Novosibirsk"	,	"Bangkok",7],
      #			["(GMT+07:00) Hanoi"	,	"Hanoi",7],
      #			["(GMT+07:00) Jakarta"	,	"Jakarta",7],
      #			["(GMT+07:00) Novosibirsk"	,	"Novosibirsk",7],
			["(GMT+08:00) Beijing, Hong Kong, Krasnoyarsk, Kuala Lumpur, Perth, Singapore, Taipei"	,	"Beijing",8],
      ...
      #			["(GMT+08:00) Urumqi"	,	"Urumqi",8],
			["(GMT+09:00) Irkutsk, Osaka, Sapporo, Seoul, Tokyo"	,	"Irkutsk",9],
      #			["(GMT+09:00) Osaka"	,	"Osaka",9],
      #			["(GMT+09:00) Sapporo"	,	"Sapporo",9],
      #			["(GMT+09:00) Seoul"	,	"Seoul",9],
      #			["(GMT+09:00) Tokyo"	,	"Tokyo",9],
			#["(GMT+09:30) Adelaide"	,	"Adelaide",9.5],
			#["(GMT+09:30) Darwin"	,	"Darwin",9.5],
			["(GMT+10:00) Brisbane, Canberra, Hobart, Melbourne, Port Moresby, Sydney, Yakutsk"	,	"Brisbane",10],
      ...
      #			["(GMT+10:00) Yakutsk"	,	"Yakutsk",10],
			["(GMT+11:00) New Caledonia, Vladivostok"	,	"New Caledonia",11],
      #			["(GMT+11:00) Vladivostok"	,	"Vladivostok",11],
			["(GMT+12:00) Auckland, Fiji, Kamchatka, Magadan, Marshall Is., Solomon Is., Wellington"	,	"Auckland",12],
      ...
      #			["(GMT+12:00) Wellington"	,	"Wellington",12],
			["(GMT+13:00) Nuku'alofa"	,	"Nuku'alofa",13]]
...
=end
  ...
  def get_correct_owner_id
    return 0 if usertype == 'accountant' or usertype == 'admin'
    return id if usertype == 'reseller'
    return owner_id
  end
  ...
  def blocked?; self.blocked == 1; end
  def save_with_balance; @save_with_balance_record; end
end
