class SmsMessage < ActiveRecord::Base
  ...
  def sms_send(user, user_tariff, number, lcr, sms_numbers, message, options = {})
    ...
    unless self.sms_set_rates_and_user(user, sms_numbers, user_tariff, number)
      return false
    end
  ...
  def sms_set_rates_and_user(user, sms_numbers, user_tariff, number)
    ...
    freze_user_balance_for_sms(user, self.user_price)
    if user.owner_id != 0
      ...
      freze_user_balance_for_sms(reseller, self.reseller_price)
    end
