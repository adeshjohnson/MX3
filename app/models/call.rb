...
', '') REGEXP '^[0-9]+$' = 0 and not_found_in_db = 1")
  ...
  def Call.insert_cdrs_from_csv(name, options)
    ...
    for r in res
      ...
      call = Call.new(:billsec => billsec, :dst => CsvImportDb.clean_value(r["col_#{options[:imp_dst]}"].to_s).gsub(/[^0-9]/, ""), :calldate => r["col_#{options[:imp_calldate]}"], :card_id => 0, :uniqueid => "", :channel => "", :dcontext => "")
