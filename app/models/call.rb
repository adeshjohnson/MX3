...
=end
  ...
  def Call::summary_by_terminator(cond, terminator_cond, order_by, user)
    if user.usertype == "reseller"
      ...
      provider_price = SqlExport.replace_price('SUM(calls.reseller_price)',{:reference=>  'provider_price'})
      ...
      provider_price = SqlExport.replace_price('SUM(calls.provider_price)', {:reference=> 'provider_price'})
    end
...
=end
  ...
  def Call::summary_by_originator(cond, terminator_cond, order_by, user)
    if user.usertype == "reseller"
      cond << "users.owner_id = #{user.id}"
      originator_billsec= "IF(calls.user_billsec IS NULL AND calls.disposition = 'ANSWERED', 0, calls.user_billsec) AS 'originator_billsec'"
      originator_price = "IF(calls.user_price IS NULL AND calls.disposition = 'ANSWERED', 0, #{SqlExport.replace_price('calls.user_price')}) AS 'originator_price'"
    else
      ...
      originator_price = "SUM(IF(owner_id = 0 AND calls.disposition = 'ANSWERED', IF(calls.user_price IS NULL, 0, #{SqlExport.replace_price('calls.user_price')}), if(calls.reseller_price IS NULL, 0, #{SqlExport.replace_price( 'calls.reseller_price')}))) AS 'originator_price'"
    end
...
=end
  ...
  def Call.last_calls(options={})
    ...
    ['user_price', 'did_price', 'did_inc_price', 'user_rate', 'reseller_rate', 'reseller_price', 'did_prov_price'].each{|co| select << "(#{co} * #{options[:exchange_rate]} ) AS #{co}_exrate"}
    ...
    if options[:current_user].usertype == 'reseller'
      if options[:current_user].reseller_allow_providers_tariff?
        select << "(IF(providers.common_use=1, reseller_rate, provider_rate) * #{options[:exchange_rate]} ) AS provider_rate_exrate"
        select << "(IF(providers.common_use=1, reseller_price + did_price + did_inc_price, provider_price) * #{options[:exchange_rate]} ) AS provider_price_exrate"
        select << "((user_price * #{options[:exchange_rate]})-(IF(providers.common_use=1, reseller_price + did_price + did_inc_price, provider_price) * #{options[:exchange_rate]} )) AS profit"
      else
        select << "(user_price-reseller_price) * #{options[:exchange_rate]} AS profit"
      end
    else
      select << "(provider_rate* #{options[:exchange_rate]}) AS provider_rate_exrate "
      select << "(provider_price * #{options[:exchange_rate]} ) AS provider_price_exrate"
      select << "(IF(resellers.id > 0,(reseller_price * #{options[:exchange_rate]}),((user_price + did_price + did_inc_price) * #{options[:exchange_rate]}))-(provider_price * #{options[:exchange_rate]})) AS profit"
    end
  ...
  def Call.last_calls_total_stats(options={})
    ...
    if options[:current_user].usertype == 'reseller'
      if options[:current_user].reseller_allow_providers_tariff?
        prov_price = "SUM(IF(providers.common_use=1, reseller_price + did_price + did_inc_price, provider_price)) * #{options[:exchange_rate].to_f} as total_provider_price"
        profit = "SUM( user_price -(IF(providers.common_use=1, reseller_price + did_price + did_inc_price, provider_price) ) )* #{options[:exchange_rate].to_f} AS total_profit"
      else
        prov_price = "SUM(provider_price)* #{options[:exchange_rate].to_f} as total_provider_price"
        profit = "SUM(user_price-reseller_price)* #{options[:exchange_rate].to_f} AS total_profit"
      end
    else
      prov_price = "SUM(provider_price)* #{options[:exchange_rate].to_f} as total_provider_price"
      profit = "SUM((IF(resellers.id > 0,(reseller_price),(user_price + did_price + did_inc_price))-(provider_price)) )* #{options[:exchange_rate].to_f} AS total_profit"
    end
    Call.find(
      :first,
      :select => "
                 ...
                 SUM(user_price + did_price + did_inc_price) * #{options[:exchange_rate].to_f} as total_user_price_with_dids,
                 ...
                 SUM(reseller_price + did_price + did_inc_price) * #{options[:exchange_rate].to_f} as total_reseller_price_with_dids,
  ...
  def Call.last_calls_csv(options={})
    ...
    s <<  SqlExport.column_escape_null(SqlExport.nice_date('calls.calldate', {:format=>format, :tz=>options[:current_user].time_zone}), "calldate2")
    ...
    if options[:current_user].usertype == "admin" or options[:current_user].usertype == "accountant"
      ...
      if options[:can_see_finances]
        s << SqlExport.replace_dec("(IF(calls.provider_rate IS NULL, 0, calls.provider_rate) * #{options[:exchange_rate]} )", options[:collumn_dem],'provider_rate')
        s << SqlExport.replace_dec("(IF(calls.provider_price IS NULL, 0, calls.provider_price) * #{options[:exchange_rate]} )",options[:collumn_dem],'provider_price')
      end
      if (defined?(RS_Active) and RS_Active.to_i == 1)
        ...
        if options[:can_see_finances]
          s << SqlExport.replace_dec("(IF(calls.reseller_rate IS NULL, 0 , calls.reseller_rate) * #{options[:exchange_rate]} ) ",options[:collumn_dem],'reseller_rate')
          s << SqlExport.replace_dec("(IF(calls.reseller_price IS NULL, 0 , calls.reseller_price) * #{options[:exchange_rate]} ) ",options[:collumn_dem],'reseller_price')
        end
      end
      ...
      if options[:can_see_finances]
        s << SqlExport.replace_dec("(IF(calls.user_rate IS NULL, 0, calls.user_rate) * #{options[:exchange_rate]} )",options[:collumn_dem],'user_rate')
        s << SqlExport.replace_dec("(IF(calls.user_price IS NULL, 0, calls.user_price) * #{options[:exchange_rate]} ) ",options[:collumn_dem],'user_price')
      end
      s << "IF(dids.did IS NULL, '' , dids.did) AS 'did'"
      if options[:can_see_finances]
        s << SqlExport.replace_dec("(IF(calls.did_prov_price IS NULL, 0, calls.did_prov_price) * #{options[:exchange_rate]} ) ",options[:collumn_dem], 'did_prov_price')
        s << SqlExport.replace_dec("(IF(calls.did_inc_price IS NULL, 0, calls.did_inc_price) * #{options[:exchange_rate]} ) ",options[:collumn_dem], 'did_inc_price')
        s << SqlExport.replace_dec("(IF(calls.did_price IS NULL, 0 , calls.did_price) * #{options[:exchange_rate]} ) ",options[:collumn_dem], 'did_price')
      end
    end
    if options[:current_user].show_billing_info == 1 and options[:can_see_finances]
      if options[:current_user].usertype == 'reseller'
        if options[:current_user].reseller_allow_providers_tariff?
          ...
          if options[:can_see_finances]
            #if reseller pro - change common use provider price, rate to reseller tariff rate, price
            s << SqlExport.replace_dec("(IF(calls.provider_rate IS NULL, 0, (IF(providers.common_use=1, calls.reseller_rate, calls.provider_rate))) * #{options[:exchange_rate]} )",options[:collumn_dem], 'provider_rate')
            s << SqlExport.replace_dec("(IF(calls.provider_price IS NULL, 0, (IF(providers.common_use=1, calls.reseller_price + calls.did_price + calls.did_inc_price,  calls.provider_price))) * #{options[:exchange_rate]} ) ",options[:collumn_dem],  'provider_price')
          end
        end
        s << SqlExport.replace_dec("(IF(calls.reseller_rate IS NULL, 0, calls.reseller_rate) * #{options[:exchange_rate]} ) ",options[:collumn_dem], 'reseller_rate')
        s << SqlExport.replace_dec("(IF((calls.reseller_price + calls.did_price + calls.did_inc_price) != 0 , (calls.reseller_price + calls.did_price + calls.did_inc_price), 0) * #{options[:exchange_rate]} ) ",options[:collumn_dem], 'reseller_price')
        s << "IF(#{options[:current_user].id} IN (users.id, users.owner_id ),IF(calls.card_id = 0 ,CONCAT(IF(users.first_name IS NULL, '', users.first_name), ' ', IF(users.last_name IS NULL, '', users.last_name)), CONCAT('Card#', IF(cards.number IS NULL, '', cards.number))), ' ' ) as 'user'"
        s << SqlExport.replace_dec("(IF(calls.user_rate IS NULL, 0, calls.user_rate) * #{options[:exchange_rate]} ) ",options[:collumn_dem], 'user_rate')
        s << SqlExport.replace_dec("(IF((calls.user_price + calls.did_price + calls.did_inc_price) != 0 , (calls.user_price + calls.did_price + calls.did_inc_price), 0) * #{options[:exchange_rate]} ) ",options[:collumn_dem], 'user_price')
      end
      if options[:current_user].usertype == 'user'
        s << SqlExport.replace_dec("((calls.user_price + calls.did_price + calls.did_inc_price) * #{options[:exchange_rate]} ) ", options[:collumn_dem], "user_price")
      end
    end
    ...
    if options[:current_user].usertype == "admin" or options[:current_user].usertype == "accountant"
      if options[:can_see_finances]
        if (defined?(RS_Active) and RS_Active.to_i == 1)
          s << SqlExport.replace_dec("(IF(resellers.id > 0,(calls.reseller_price * #{options[:exchange_rate]}),(calls.user_price * #{options[:exchange_rate]}))-(calls.provider_price * #{options[:exchange_rate]}))",options[:collumn_dem],'profit')
        else
          s << SqlExport.replace_dec("(calls.user_price * #{options[:exchange_rate]})-(calls.provider_price * #{options[:exchange_rate]})",options[:collumn_dem],'profit')
        end
      end
    elsif options[:current_user].usertype == 'reseller'
      if options[:can_see_finances]
        if options[:current_user].reseller_allow_providers_tariff?
          s << SqlExport.replace_dec("((calls.user_price * #{options[:exchange_rate]})-(IF(providers.common_use=1, calls.reseller_price + calls.did_price + calls.did_inc_price, calls.provider_price) * #{options[:exchange_rate]} ))",options[:collumn_dem],  'profit')
        else
          s << SqlExport.replace_dec("(calls.user_price-calls.reseller_price) * #{options[:exchange_rate]}",options[:collumn_dem],  'profit')
        end
      end
      s << "IF(dids.did IS NULL or dids.reseller_id != #{options[:current_user].id}, '' , dids.did) AS 'did'"
    end
    ...
    sql += " FROM (SELECT #{s.join(', ')}
             FROM calls
             LEFT JOIN destinations ON (destinations.prefix = IF(calls.prefix IS NULL, '', calls.prefix))
             LEFT JOIN directions ON (directions.code = destinations.direction_code)
             #{jn.join(' ')}  WHERE #{ ActiveRecord::Base.sanitize_sql([cond.join(' AND '), *var])} ORDER BY #{options[:order]} ) as C"
  ...
  def Call.calls_for_laod_stats(options={})
    ...
    if options[:s_reseller].to_i != -1
      cond << 'reseller_id = ? ' ; var << options[:s_reseller]
    end
    ...
    c2 = Call.find(:all, :conditions=>[cond.join(' AND ').to_s] + var, :select=>'calldate, duration', :group=>'calldate')
    cond << "disposition = 'ANSWERED'"
    calls1 = Call.find(:all, :conditions=>[cond.join(' AND ').to_s] + var, :select=>'calldate, duration', :group=>'calldate')
    return calls1, c2
  end
  ...
  def Call.country_stats(options={})
    ...
    if options[:current_user].usertype == "reseller"
      cond << "(calls.reseller_id = ? OR calls.user_id = ? OR calls.dst_user_id = ?)"
      var += [options[:current_user].id, options[:current_user].id, options[:current_user].id]
      user_price = SqlExport.replace_price('calls.user_price')
      provider_price =SqlExport.replace_price('calls.reseller_price')
    else
      user_price = SqlExport.replace_price('IF(calls.reseller_id = 0, user_price, reseller_price)')
      provider_price =SqlExport.replace_price('calls.provider_price')
    end
    ...
    calls_all = Call.find(:all,
      ...
      :joins=>'LEFT JOIN destinations ON (destinations.prefix = calls.prefix) JOIN destinationgroups ON (destinations.destinationgroup_id = destinationgroups.id)')
  ...
  def Call.hangup_cause_codes_stats(options={})
    ...
    sql = "SELECT calls_hc.hc_code, calls_hc.calls, hangupcausecodes.id, hangupcausecodes.code, hangupcausecodes.description FROM(
              SELECT calls.hangupcause AS 'hc_code', count(calls.id) AS 'calls' FROM calls #{des} WHERE #{ ActiveRecord::Base.sanitize_sql([cond.join(' AND '), *var])} GROUP BY hc_code) AS calls_hc
    ...
    day_calls = Call.find_by_sql(
      "SELECT * FROM (SELECT * FROM (SELECT * FROM (#{date_period.join(" UNION ")}) AS v) AS d) AS u 
        LEFT JOIN (SELECT DATE(calldate) as call_date, #{SqlExport.nice_date('DATE(calldate)',  {:reference=> 'call_date_formated', :format=>format, :tz=>options[:current_user].time_zone})}, SUM(IF(calls.hangupcause = '16', 1,0)) as 'calls', SUM(IF(calls.hangupcause != '16', 1,0)) as 'b_calls' FROM calls
                    ...
                    WHERE #{ ActiveRecord::Base.sanitize_sql([(cond+["calls.hangupcause != ''"]).join(' AND '), *var])}
  ...
  def Call.country_stats_csv(options={})
    ...
    jn = ['LEFT JOIN destinations ON (destinations.prefix = calls.prefix)','JOIN destinationgroups ON (destinations.destinationgroup_id = destinationgroups.id)']
    if options[:s_user].to_i != -1
    ...
    sql += " FROM (SELECT #{s.join(', ')} FROM calls #{jn.join(' ')}  WHERE #{ ActiveRecord::Base.sanitize_sql([cond.join(' AND '), *var])} GROUP BY destinationgroups.id ORDER BY destinationgroups.name ASC, destinationgroups.desttype ASC ) as C"
  ...
  def Call.cardgroup_aggregate(options={})
    ...
    if options[:csv].to_i == 1
      ...
      sql += " FROM (SELECT #{s.join(', ')} FROM calls #{jn.join(' ')}  WHERE #{ ActiveRecord::Base.sanitize_sql([cond.join(' AND '), *var])} #{group}  #{order}  ) as C"
      ...
      sql = "SELECT #{s.join(', ')} FROM calls #{jn.join(' ')}  WHERE #{ ActiveRecord::Base.sanitize_sql([cond.join(' AND '), *var])}  #{group}  #{order} "
...
=end
  ...
  def Call.last_calls_parse_params(options={})
    ...
    if options[:provider]
      cond << "calls.provider_id = ?"
      var << options[:provider].id
    end
