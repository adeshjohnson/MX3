class Provider < ActiveRecord::Base
  ...
  before_save :before_save_timeout, :check_location_id
  ...
  def prune_device_in_servers(dev_username)
    exceptions = []
    for server in self.servers
      begin
        server.prune_peer(dev_username)
      rescue Exception => e
        exceptions << e
      end
    end
  ...
  def Provider.providers_order_by(options)
    ...
    when "name" :     order_by = "providers.name"
    when "id" :      order_by = "providers.id"
    when "tech"  :  order_by = "providers.tech"
    when "channel" :  order_by = "providers.channel"
    when "login" :  order_by = "providers.login"
    when "password" :  order_by = "providers.password"
    when "server_ip" :  order_by = "providers.server_ip"
    when "tariff" :  order_by = "tariffs.name"
    else
  ...
  def old_register; @old_register_record; end
  def old_register_extension; @old_register_extension_record; end
  def old_register_line; @old_register_line_record; end
