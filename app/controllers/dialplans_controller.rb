class DialplansController < ApplicationController
  ...
  verify :method => :post, :only => [ :destroy, :create, :update, :did_assign_to_dp ],
  ...
  def dialplans
    ...
    @ccdps = []
    @ccdps = current_user.dialplans.find(:all, :conditions => "dptype = 'callingcard'", :order => "name ASC") if cc_active?
  ...
  def edit
    ...
    @cbdevices =  Device.find(:all, :conditions => "user_id != -1 AND users.owner_id = #{current_user.id}", :include=>[:user], :order => "name ASC")
  ...
  def dialplans_device_ajax
    ...
    @device = []
    if params[:id]
      @device = Device.find(:all,:conditions =>['user_id =?',params[:id]])
    end
    render :layout => false
  end
