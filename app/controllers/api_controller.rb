...
=end
  ...
  def payments_list
    ...
    allow , values = API.check_params_with_key(params, request)
    doc = Builder::XmlMarkup.new( :target => out_string = "", :indent => 2 )
    ...
    if allow == true
      if @user
        ...
        if @user.usertype == 'accountant' or @user.usertype == 'admin'
          @owner_id = 0
        elsif @user.usertype ==  'reseller'
          @owner_id = @user.id
        else
          #user can see only his, so overwrite s_user_id
          @owner_id = @user.owner_id
          @options[:s_user_id] = @user.id
        end
...
=end
    ...
    if allow == true
      ...
      if @user_logged
        if @user_logged.usertype == 'admin'
          @user = User.find(:first, :conditions => "id = #{values[:user_id]}") if values [:user_id]
          ...
          if values [:user_id]
            ...
            @user = User.find(:first, :conditions => "id = #{values[:user_id]} AND owner_id = #{owner.to_i}")
          end
  ...
  def user_devices
    ...
    if @user
      ...
      @devices = Device.find(:all, :conditions => "user_id = #{session[:user_id]}", :order => "name")
    ...
    if allow == true
      ...
      if @user_logged
        ...
        if @user_logged.usertype.to_s == "reseller"
          user = User.find(:first, :conditions=>["id=? and owner_id =?",@options[:s_user], @user_logged.id]) if @options[:s_user] != "all" and !@options[:s_user].blank?
        ...
        if user or @options[:s_user] == "all"
          ...
          doc.page{
            ...
            doc.calls_stat{
              ...
              doc.show_direction(@options[:s_direction])
      ...
      if defined?(MA_Active) and MA_Active == 1
        if values.has_keys?(:monitoring_id, :users, :block, :email, :mtype)
          ...
          if monitoring
                      ...
                      if monitoring.block_user?
                        ...
                        if monitoring.send_email?
                          if Confline.get('Email_Sending_Enabled').value.to_i == 1
                            admin = User.find_by_id(0)
                            if admin.address and !admin.address.email.empty?
                              status << " email sent"
                            else
                              status << " admin has no email set"
                            end
                          else
                        ...
                        Action.add_action_hash(0, # for admin
            ...
            unless users.empty?
              email = Email.find(:first, :conditions => { :name => 'monitoring_activation', :owner_id => 0 })
              user = User.find_by_id(0)

              variables = Email.email_variables(user, nil, { :monitoring => monitoring, :monitoring_type => _(monitoring.monitoring_type), :monitoring_users_list => users })
...
=end
  ...
  def user_update_api
    ...
    if allow == true
      check_user(params[:u],params[:p])
      if @user
        if @user.usertype == 'user' or (@user.usertype == 'accountant' and values[:user_id].to_i == 0)
          ...
          if user_u
            ...
            params[:address] = {}
            ['address', 'city', 'postcode', 'county', 'mob_phone', 'fax', 'direction_id', 'phone', 'email', 'satate'].each_with_index{|s,i|
      ...
      if tariff[:name] and !tariff[:name].empty?
        ...
        if !try_find_tariff
          ...
          if tariff_new.save
            ...
            if find_tariff_id.owner_id.to_i == user.id or user.usertype.to_s == 'admin'
...
=end
  ...
  def check_allow_api
    if Confline.get_value("Allow_API").to_i != 1
      doc = Builder::XmlMarkup.new( :target => out_string = "", :indent => 2 )
      doc.status("API Requests are disabled")
      send_xml_data(out_string)
    end
  end
...
=end
  ...
  def check_send_method
    if request.get? and Confline.get_value("Allow_GET_API").to_i != 1
      doc = Builder::XmlMarkup.new( :target => out_string = "", :indent => 2 )
      doc.status("GET Requests are disabled")
      send_xml_data(out_string)
    end
  end
...
=end
  ...
  def send_xml_data(out_string, test = 0, name = "mor_api_response.xml")
    if test.to_i == 1
...
=end
  def check_user(login,password)
    if login.to_s.length > 0 and password.to_s.length > 0 and @user = User.find(:first, :conditions => ["username = ? and password = ?", login, Digest::SHA1.hexdigest(password)])
      if @user
        User.current = @user
      end
      return @user
