...
sql = "SELECT services.name as serv_name , users.first_name, users.last_name, users.username, subscriptions.*, devices.device_type,  devices.name, devices.extension, devices.istrunk, providers.device_id AS provider FROM subscriptions 
  ...
  def subscription_create
    ...
    @sub.activation_start = user_time_from_params(*params['activation_start'].values) 
    @sub.activation_end   = user_time_from_params(*params['activation_end'].values)
    ...
    valid_membership_time = @user_registered <= @sub.activation_start.strftime("%Y-%m-%d %H:%M:%S").to_time.to_i
