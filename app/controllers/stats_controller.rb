class StatsController < ApplicationController
  ...
  before_filter :check_reseller_in_providers, :only=>[:providers, :providers_stats]
  ...
  def show_user_stats
    ...
    #end
    @all_profit =  Currency.count_exchange_prices({:exrate=>exrate, :prices=>[sum[0]["price"].to_f]}).to_f - @all_prov_price.to_f
...
=end
  def last_calls_stats
    ...
    when 'csv'
      ...
      options[:collumn_separator], options[:collumn_dem] = current_user.csv_params
      options[:current_user] = current_user
...
=end
  ...
  def country_stats
    ...
    if params[:csv].to_i == 0
      ...
      @users = User.find_all_for_select(corrected_user_id,{:exclude_owner=>true})
      @calls, @longest_time, @profit, @incomes , @calls_all = Call.country_stats({:user_id=>params[:user_id], :current_user=>current_user, :a1=>session_from_date,:a2=>session_till_date})
    else
      ...
      filename = Call.country_stats_csv({:collumn_separator => Confline.get_csv_separator(settings_owner_id), :s_user=>@user_id, :current_user=>current_user, :from=>session_from_date, :till=>session_till_date, :nice_number_digits=>session[:nice_number_digits], :test=>params[:test].to_i, :hide_finances => !can_see_finances?})
      filename = load_file_through_database(filename) if Confline.get_value("Load_CSV_From_Remote_Mysql").to_i == 1
      if filename
        filename = archive_file_if_size(filename, "csv", Confline.get_value("CSV_File_size").to_f)
        if params[:test].to_i != 1
...
=begin
    if @device
       calls = @device.calls(call_type, date_from, date_till)
    else
       if call_type.last(4).to_s == "_inc"
          @call_type = call_type.to_s.split("_inc")
          @inc_call_type = " AND disposition = '#{@call_type[0]}' "
          if @call_type.to_s == "all"
              @inc_call_type = ""
          end
          calls = Call.find_by_sql("SELECT calls.* FROM calls JOIN devices ON (devices.id = dst_device_id) WHERE (calls.card_id = 0 AND devices.user_id = #{user.id} #{@inc_call_type} AND callertype = 'Outside' AND ((calldate BETWEEN '#{date_from.to_s}' AND '#{date_till.to_s}' AND processed = 1) OR (processed = 0))) ORDER BY calldate DESC")
       else
          calls = user.calls(call_type, date_from, date_till)
       end
    end
=end
    ...
    pdf.text(_('Period') + ": " + date_from + "  -  " + date_till,        {:font_size => 10, :top => 55, :alignment => :left})
    pdf.text(_('Currency') + ": #{session[:show_currency]}",              {:font_size => 8,  :top => 84, :alignment => :left})
    ...
    page_calls = 1
    #table header
    pdf = PdfGen::Generate.call_list_to_pdf_header(pdf, @direction, session[:usertype], i, options)
    #/table header
    #page
    ...
    for call in calls
      ...
      pdf.text(call.calldate.strftime("%Y-%m-%d %H:%M:%S"),{:left=>options[:dat_x] , :top=>options[:ystart]+(page_calls % options[:calls_per_page] * options[:xdelta]), :font_size =>options[:fontsize]})
      pdf.text(call.src,                                   {:left=>options[:caf_x] , :top=>options[:ystart]+(page_calls % options[:calls_per_page] * options[:xdelta]), :font_size =>options[:fontsize]})
      ...
      pdf.text(nice_time(billsec),                         {:left=>options[:dur_x] , :top=>options[:ystart]+(page_calls % options[:calls_per_page] * options[:xdelta]), :font_size =>options[:fontsize]})
      pdf.text(call.disposition,                           {:left=>options[:han_x] , :top=>options[:ystart]+(page_calls % options[:calls_per_page] * options[:xdelta]), :font_size =>options[:fontsize]})

      if session[:usertype] == "admin"
        if @direction == "incoming"
          pdf.text(nice_number(@rate_did_pr),                               {:left=> options[:pri_x],  :top=> options[:ystart]+(page_calls % options[:calls_per_page] * options[:xdelta]), :font_size =>options[:fontsize]})
          pdf.text(nice_number(@rate_did_ic),                               {:left=> options[:pri2_x], :top=> options[:ystart]+(page_calls % options[:calls_per_page] * options[:xdelta]), :font_size =>options[:fontsize]})
          pdf.text(nice_number(@rate_did_ow),                               {:left=> options[:pri3_x], :top=> options[:ystart]+(page_calls % options[:calls_per_page] * options[:xdelta]), :font_size =>options[:fontsize]})
          pdf.text(nice_number(@rate_did_pr + @rate_did_ic + @rate_did_ow ),{:left=> options[:pri4_x], :top=> options[:ystart]+(page_calls % options[:calls_per_page] * options[:xdelta]), :font_size =>options[:fontsize]})
        else
          pdf.text(nice_number(@rate_cur3),                                                        {:left=> options[:pri_x],  :top=> options[:ystart]+(page_calls % options[:calls_per_page] * options[:xdelta]), :font_size =>options[:fontsize]})
          pdf.text(nice_number(@rate_prov),                                                        {:left=> options[:pri2_x], :top=> options[:ystart]+(page_calls % options[:calls_per_page] * options[:xdelta]), :font_size =>options[:fontsize]})
          pdf.text(nice_number(@rate_cur3.to_f - @rate_prov.to_f),                                 {:left=> options[:pri3_x], :top=> options[:ystart]+(page_calls % options[:calls_per_page] * options[:xdelta]), :font_size =>options[:fontsize]})
          pdf.text(nice_number(@rate_cur3.to_f != 0.to_f ? ((@rate_cur3.to_f - @rate_prov.to_f)/ @rate_cur3.to_f) *100 : 0) + " %",{:left=> options[:pri4_x], :top=> options[:ystart]+(page_calls % options[:calls_per_page] * options[:xdelta]), :font_size =>options[:fontsize]}) if @rate_cur3.to_f != 0
          pdf.text(nice_number(@rate_prov.to_f != 0.to_f ? ((@rate_cur3.to_f / @rate_prov.to_f) *100)-100 : 0) + " %",             {:left=> options[:pri5_x], :top=> options[:ystart]+(page_calls % options[:calls_per_page] * options[:xdelta]), :font_size =>options[:fontsize]}) if @rate_prov.to_f != 0
        end
      end
      ...
      if session[:usertype] == "reseller"
        if @direction == "incoming"
          #          pdf.text(nice_number(@rate_did_pr),                               {:left=> options[:pri_x],  :top=> options[:ystart]+(page_calls % options[:calls_per_page] * options[:xdelta]), :font_size =>options[:fontsize]})
          #          pdf.text(nice_number(@rate_did_ic),                               {:left=> options[:pri2_x], :top=> options[:ystart]+(page_calls % options[:calls_per_page] * options[:xdelta]), :font_size =>options[:fontsize]})
          pdf.text(nice_number(@rate_did_ow),                               {:left=> options[:pri_x], :top=> options[:ystart]+(page_calls % options[:calls_per_page] * options[:xdelta]), :font_size =>options[:fontsize]})
          #          pdf.text(nice_number(@rate_did_pr + @rate_did_ic + @rate_did_ow ),{:left=> options[:pri4_x], :top=> options[:ystart]+(page_calls % options[:calls_per_page] * options[:xdelta]), :font_size =>options[:fontsize]})
        else
          pdf.text(nice_number(@rate_cur3),                                                        {:left=> options[:pri_x],  :top=> options[:ystart]+(page_calls % options[:calls_per_page] * options[:xdelta]), :font_size =>options[:fontsize]})
          pdf.text(nice_number(@rate_prov),                                                        {:left=> options[:pri2_x], :top=> options[:ystart]+(page_calls % options[:calls_per_page] * options[:xdelta]), :font_size =>options[:fontsize]})
    ...
    #Totals
    pdf.text(_('Total'),               {:left=> 40,    :top=>options[:total_possition], :font_size =>options[:fontsize] + options[:header_add_size]})
    pdf.text(nice_time(total_billsec),{:left=> options[:dur_x], :top=>options[:total_possition], :font_size =>options[:fontsize] + options[:header_add_size]})
    if session[:usertype] == "admin" or session[:usertype] == "reseller"
      if @direction == "incoming"
        pdf.text(nice_number(total_did_provider),{:left=>options[:pri_x],  :top=>options[:total_possition] , :font_size =>options[:fontsize] + options[:header_add_size]})
        pdf.text(nice_number(total_did_inc),     {:left=>options[:pri2_x], :top=>options[:total_possition] , :font_size =>options[:fontsize] + options[:header_add_size]})
        pdf.text(nice_number(total_did_own),     {:left=>options[:pri3_x], :top=>options[:total_possition] , :font_size =>options[:fontsize] + options[:header_add_size]})
        pdf.text(nice_number(total_did_prof),    {:left=>options[:pri4_x], :top=>options[:total_possition] , :font_size =>options[:fontsize] + options[:header_add_size]})
      else
        pdf.text(nice_number(total_price),                          {:left=>options[:pri_x] , :top=> options[:total_possition], :font_size =>options[:fontsize] + options[:header_add_size]})
        pdf.text(nice_number(total_prov_price),                     {:left=>options[:pri2_x], :top=> options[:total_possition], :font_size =>options[:fontsize] + options[:header_add_size]})
        pdf.text(nice_number(total_prfit),                          {:left=>options[:pri3_x], :top=> options[:total_possition], :font_size =>options[:fontsize] + options[:header_add_size]})
        if total_price.to_f != 0
          pdf.text(nice_number(total_price.to_f != 0.to_f ? (total_prfit / total_price.to_f) * 100 : 0), {:left=>options[:pri4_x], :top=> options[:total_possition], :font_size =>options[:fontsize] + options[:header_add_size]})
        else
          pdf.text(nice_number(0), {:left=>options[:pri4_x], :top=> options[:total_possition], :font_size =>options[:fontsize] + options[:header_add_size]})
        end
        if total_prov_price.to_f != 0
          pdf.text(nice_number(total_prov_price.to_f != 0 ? ((total_price.to_f / total_prov_price.to_f) *100)-100 : 0) + " %", {:left=>options[:pri5_x], :top=> options[:total_possition], :font_size =>options[:fontsize] + options[:header_add_size]})
        else
          pdf.text(nice_number(0) + " %", {:left=>options[:pri5_x], :top=> options[:total_possition], :font_size =>options[:fontsize] + options[:header_add_size]})
        end
      end
    else
      if @direction != "incoming"
  ...
  def profit
    ...
    conditions << "calls.calldate BETWEEN '#{session_from_date} 00:00:00' AND '#{session_till_date} 23:59:59'"
    select = ["SUM(IF(calls.billsec > 0, calls.billsec, CEIL(calls.real_billsec) )) AS 'billsec'"]
    if session[:usertype] == "reseller"
      select += [SqlExport.replace_price('SUM(calls.user_price)',{:reference=> 'user_price'}), SqlExport.replace_price('SUM(calls.reseller_price)', {:reference=> 'provider_price'})]
      conditions << "calls.reseller_id = #{session[:user_id].to_i}"
    else
      select += [SqlExport.replace_price('SUM(IF(calls.reseller_id > 0, calls.reseller_price, calls.user_price))', {:reference=> 'user_price'}) , SqlExport.replace_price('SUM(calls.provider_price)',{:reference=> 'provider_price'})]
    end
    total = Call.find(:all, :select => select.join(", "),:joins => "LEFT JOIN users ON (users.id = calls.user_id)", :conditions => (conditions +["disposition = 'ANSWERED'"]).join(" AND "))[0]
    ...
    total = Call.find(:all, :select => select_total.join(", "), :conditions =>conditions.join(" AND ") )
    ...
    if @total_call_price != 0 && @total_answered_calls != 0
      ...
      if session[:usertype] == "reseller"
        res = Call.find(:all,
          :select => "#{SqlExport.replace_price('SUM(calls.user_price)', {:reference=> 'price'})}, SUM(IF(calls.billsec > 0, calls.billsec, CEIL(calls.real_billsec) )) AS 'duration', COUNT(DISTINCT(calls.user_id)) AS 'users'",
          :joins => "LEFT JOIN users ON (users.id = calls.user_id)",
          :conditions => (conditions + ["calls.disposition = 'ANSWERED'"]).join(" AND "))
      else
        res = Call.find(:all,
          :select => "#{SqlExport.replace_price('SUM(IF(calls.reseller_id > 0, calls.reseller_price, calls.user_price))',{:reference=>  'price'})}, SUM(IF(calls.billsec > 0, calls.billsec, CEIL(calls.real_billsec) )) AS 'duration', COUNT(DISTINCT(calls.user_id)) AS 'users'",
          :joins => "LEFT JOIN users ON (users.id = calls.user_id)",
          :conditions => (conditions + ["calls.disposition = 'ANSWERED'"]).join(" AND "))
      end
...
=end
  ...
  def generate_profit_pdf
    ...
    options = {
      ...
      #vertical positions
      :first_line_y => 203,
      :last_line_y_admin => 600,
      :last_line_y_reseller => 520
      #      :rec_h => 25,
      #      :rec1_y => 305,
      #      :rec2_y => 405,
      #      :rec3_y => 508,
      ...
      #      :rec5_y => 548,
      #colors
      #      :color_1 => [1.0,  0.88, 1.0], # pink
      #      :color_2 => [0.89, 0.88, 0.87] # gray
    }
    pdf = PDF::Wrapper.new(:paper => :A4)
    pdf.font(options[:font_bold])
    pdf.text(_('PROFIT_REPORT'), {:top => 50, :font_size => options[:title_fontsize], :alignment => :left})
    pdf.font(options[:font])
    pdf.text(_('Time_period') + ": " + session_from_date.to_s + " - " + session_till_date.to_s, {:top => 80 , :alignment =>:left,  :font_size => options[:title_fontsize2]})
    pdf.text(_('Counting') + ": " + user_name.to_s, {:top => 100, :alignment =>:left, :font_size => options[:title_fontsize2] })

    pdf.line(options[:col1_x], options[:first_line_y], options[:end_x], options[:first_line_y], {:line_width => options[:line_strength]})

    if session[:usertype] != 'reseller'
      pdf.line(options[:col1_x], options[:last_line_y_admin], options[:end_x], options[:last_line_y_admin], {:line_width => options[:line_strength]})
    else
      pdf.line(options[:col1_x], options[:last_line_y_reseller], options[:end_x], options[:last_line_y_reseller], {:line_width => options[:line_strength]})
    end
    line = 1
    pdf.text(_('Total_calls'),     {:top => options[:text_start]+ (line*options[:step]), :left => options[:col1_x], :font_size => options[:fontsize] + options[:header_size_add]})
    pdf.text(params[:total_calls], {:top => options[:text_start]+ (line*options[:step]), :left => options[:col2_x], :font_size => options[:fontsize]})
    line += 1
    pdf.text(_('Answered_calls'),           {:top => options[:text_start]+ (line*options[:step]), :left => options[:col1_x], :font_size => options[:fontsize] + options[:header_size_add]})
    pdf.text(params[:total_answered_calls], {:top => options[:text_start]+ (line*options[:step]), :left => options[:col2_x], :font_size => options[:fontsize]})
    pdf.text(nice_number(params[:total_answer_percent]) + " %", {:top => options[:text_start]+ (line*options[:step]), :left => options[:col3_x], :font_size => options[:fontsize]})
    pdf.text(_('Duration') + ": " + nice_time(params[:total_duration]), {:top => options[:text_start]+ (line*options[:step]), :left => options[:col4_x], :font_size => options[:fontsize]})
    pdf.text(_('Average_call_duration') + ": " + nice_time(params[:average_call_duration]), {:top => options[:text_start]+ (line*options[:step]), :left => options[:col7_x] - 20, :font_size => options[:fontsize]})
    line += 1
    pdf.text(_('No_Answer'),                {:top => options[:text_start]+ (line*options[:step]), :left => options[:col1_x], :font_size => options[:fontsize] + options[:header_size_add]})
    pdf.text(params[:total_not_ans_calls],  {:top => options[:text_start]+ (line*options[:step]), :left => options[:col2_x], :font_size => options[:fontsize]})
    pdf.text(nice_number(params[:total_not_ans_percent]) + " %", {:top => options[:text_start]+ (line*options[:step]), :left => options[:col3_x], :font_size => options[:fontsize]})
    line += 1
    pdf.text(_('Busy_calls'),           {:top => options[:text_start]+ (line*options[:step]), :left => options[:col1_x], :font_size => options[:fontsize] + options[:header_size_add]})
    pdf.text(params[:total_busy_calls], {:top => options[:text_start]+ (line*options[:step]), :left => options[:col2_x], :font_size => options[:fontsize]})
    pdf.text(nice_number(params[:total_busy_percent]) + " %", {:top => options[:text_start]+ (line*options[:step]), :left => options[:col3_x], :font_size => options[:fontsize]})
    line += 1
    pdf.text(_('Error_calls'),           {:top => options[:text_start]+ (line*options[:step]), :left => options[:col1_x], :font_size => options[:fontsize] + options[:header_size_add]})
    pdf.text(params[:total_error_calls], {:top => options[:text_start]+ (line*options[:step]), :left => options[:col2_x], :font_size => options[:fontsize]})
    pdf.text(nice_number(params[:total_error_percent]) + " %", {:top => options[:text_start]+ (line*options[:step]), :left => options[:col3_x], :font_size => options[:fontsize]})
    line += 1
    pdf.font(options[:font_bold])
    pdf.text(_('Price'),  {:top => options[:text_start] + (line*options[:step]), :left => options[:col2_x], :font_size => options[:fontsize] + options[:header_size_add]})
    pdf.text(_('Percent'),      {:top => options[:text_start] + (line*options[:step]), :left => options[:col3_x], :font_size => options[:fontsize] + options[:header_size_add]})
    pdf.text(_('Call_time'),    {:top => options[:text_start] + (line*options[:step]), :left => options[:col4_x], :font_size => options[:fontsize] + options[:header_size_add]})
    pdf.text(_('Active_users'), {:top => options[:text_start] + (line*options[:step]), :left => options[:col6_x], :font_size => options[:fontsize] + options[:header_size_add]})
    pdf.font(options[:font])
    line += 1
    pdf.text(_('Total_call_price'),                           {:top => options[:text_start]+ (line*options[:step]), :left => options[:col1_x], :font_size => options[:fontsize] + options[:header_size_add]})
    pdf.text(nice_number(params[:total_call_price]),          {:top => options[:text_start]+ (line*options[:step]), :left => options[:col2_x], :font_size => options[:fontsize]})
    pdf.text(nice_number(params[:total_percent]),             {:top => options[:text_start]+ (line*options[:step]), :left => options[:col3_x], :font_size => options[:fontsize]})
    pdf.text(nice_time(params[:total_duration]),              {:top => options[:text_start]+ (line*options[:step]), :left => options[:col4_x], :font_size => options[:fontsize]})
    pdf.text(params[:active_users].to_i,              {:top => options[:text_start]+ (line*options[:step]), :left => options[:col6_x], :font_size => options[:fontsize]})
    line += 1
    pdf.text(_('Total_call_self_price'),                   {:top => options[:text_start]+ (line*options[:step]), :left => options[:col1_x], :font_size => options[:fontsize] + options[:header_size_add]})
    pdf.text(nice_number(params[:total_call_selfprice]),   {:top => options[:text_start]+ (line*options[:step]), :left => options[:col2_x], :font_size => options[:fontsize]})
    pdf.text(nice_number(params[:total_selfcost_percent]), {:top => options[:text_start]+ (line*options[:step]), :left => options[:col3_x], :font_size => options[:fontsize]})
    line += 1
    pdf.text(_('Calls_profit'),                           {:top => options[:text_start]+ (line*options[:step]), :left => options[:col1_x], :font_size => options[:fontsize]+ options[:header_size_add]})
    pdf.text(nice_number(params[:total_profit]),          {:top => options[:text_start]+ (line*options[:step]), :left => options[:col2_x], :font_size => options[:fontsize]})
    pdf.text(nice_number(params[:total_percent_percent]), {:top => options[:text_start]+ (line*options[:step]), :left => options[:col3_x], :font_size => options[:fontsize]})
    line += 1
    pdf.text(_('Average_profit_per_call_min'),           {:top => options[:text_start]+ (line*options[:step]), :left => options[:col1_x], :font_size => options[:fontsize] + options[:header_size_add]})
    pdf.text(nice_number(params[:avg_profit_call_min]),  {:top => options[:text_start]+ (line*options[:step]), :left => options[:col2_x], :font_size => options[:fontsize]})
    line += 1
    pdf.text(_('Average_profit_per_call'),          {:top => options[:text_start]+ (line*options[:step]), :left => options[:col1_x], :font_size => options[:fontsize] + options[:header_size_add]})
    pdf.text(nice_number(params[:avg_profit_call]), {:top => options[:text_start]+ (line*options[:step]), :left => options[:col2_x], :font_size => options[:fontsize]})
    line += 1
    pdf.text(_('Average_profit_per_day'),          {:top => options[:text_start]+ (line*options[:step]), :left => options[:col1_x], :font_size => options[:fontsize] + options[:header_size_add]})
    pdf.text(nice_number(params[:avg_profit_day]), {:top => options[:text_start]+ (line*options[:step]), :left => options[:col2_x], :font_size => options[:fontsize]})
    line += 1
    pdf.text(_('Average_profit_per_active_user'),   {:top => options[:text_start]+ (line*options[:step]), :left => options[:col1_x], :font_size => options[:fontsize] + options[:header_size_add]})
    pdf.text(nice_number(params[:avg_profit_user]), {:top => options[:text_start]+ (line*options[:step]), :left => options[:col2_x], :font_size => options[:fontsize]})
    if session[:usertype] != 'reseller'
      line += 1
      pdf.font(options[:font_bold])
      pdf.text(_('Price'), {:top => options[:text_start]+ (line*options[:step]), :left => options[:col2_x], :font_size => options[:fontsize] + options[:header_size_add]})
      ...
      line += 1
      pdf.font(options[:font_bold])
      pdf.text(_('Subscriptions_profit'),          {:top => options[:text_start]+ (line*options[:step]), :left => options[:col1_x], :font_size => options[:fontsize] + options[:header_size_add]})
      pdf.font(options[:font])
      pdf.text(nice_number(params[:sub_price]),    {:top => options[:text_start]+ (line*options[:step]), :left => options[:col2_x], :font_size => options[:fontsize]})
      line += 1
      pdf.font(options[:font_bold])
      pdf.text(_('Total_profit'),             {:top => options[:text_start]+ (line*options[:step]), :left => options[:col1_x], :font_size => options[:fontsize] + options[:header_size_add]})
      pdf.font(options[:font])
      pdf.text(nice_number(params[:s_total]), {:top => options[:text_start]+ (line*options[:step]), :left => options[:col2_x], :font_size => options[:fontsize]})
    end
  ...
  def active_calls_show
    ...
    sql = "
    SELECT
    activecalls.id as ac_id, activecalls.channel as channel, activecalls.prefix, activecalls.server_id as server_id, activecalls.start_time as start_time,
    ...
    devices.id as 'device_id',devices.device_type as device_type, devices.name as device_name, devices.extension as device_extension, devices.istrunk as device_istrunk, devices.ani as device_ani,
    dst.id as dst_device_id,  dst.device_type as dst_device_type, dst.name as dst_device_name, dst.extension as dst_device_extension, dst.istrunk as dst_device_istrunk, dst.ani as dst_device_ani,
...
=end
  ...
  def dids
    ...
    left JOIN actions on (dids.id = actions.data AND actions.action = 'did_assigned')
  ...
  def calls_per_day
    ...
    #     logger.fatal  User.sistem_time_ofset.to_i
    calldate = "(calls.calldate + INTERVAL #{current_user.time_zone.to_i - User.sistem_time_ofset.to_i} HOUR)"

    sql = "SELECT EXTRACT(YEAR FROM #{calldate}) as year, EXTRACT(MONTH FROM #{calldate}) as month, EXTRACT(day FROM #{calldate}) as day, Count(calls.id) as 'calls' , SUM(IF(calls.billsec > 0, calls.billsec, CEIL(calls.real_billsec) )) as 'duration', SUM(IF(reseller_id=0, calls.user_price, calls.reseller_price)) as 'user_price', SUM(IF(reseller_id !='#{session[:user_id]}',calls.reseller_price,0)) as 'resseler_price', SUM(calls.provider_price) as 'provider_price', SUM(IF(disposition!='ANSWERED',1,0)) as 'fail'  FROM
    #{des3} calls #{des2}
    WHERE #{cond} calldate BETWEEN '#{session_from_datetime}' AND '#{session_till_datetime}' #{des}
    GROUP BY year, month, day"
    @res = ActiveRecord::Base.connection.select_all(sql)

    sql_total = "SELECT  Count(calls.id) as 'calls' , SUM(IF(calls.billsec > 0, calls.billsec, CEIL(calls.real_billsec) )) as 'duration', SUM(IF(reseller_id=0,calls.user_price,calls.reseller_price)) as 'user_price', SUM(IF(reseller_id !='#{session[:user_id]}',calls.reseller_price,0)) as 'resseler_price', SUM(calls.provider_price) as 'provider_price', SUM(IF(disposition!='ANSWERED',1,0)) as 'fail'  FROM
    #{des3} calls #{des2}
    WHERE #{cond} calldate BETWEEN '#{session_from_datetime}' AND '#{session_till_datetime}' #{des}"
...
=end
  ...
  def action_log
    ...
    params[:user_id]   ? @options[:s_user] = params[:user_id].to_s     : (params[:clean]) ? @options[:s_user] = -1   : (@options[:s_user])? @options[:s_user] = session[:action_log_stats_options][:s_user] : @options[:s_user] = -1
    ...
    cond_arr = []
    join = ""
    session[:usertype] == 'admin' ? cond = [] : cond = ["actions.action NOT in ('bad_login')"]

    if session[:usertype] == 'reseller'
      cond << "users.owner_id = ?"
      cond_arr << session[:user_id]
    end

    if @options[:s_int_ch].to_i != 1
