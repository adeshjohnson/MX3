class PaymentsController < ApplicationController
  ...
  def confirm_payment
    ...
    user.balance += round_to_cents((["paypal", "osmp_osmp"].include?(@payment.paymenttype) ? @payment.amount : @payment.gross) * exchange_rate).to_f
  ...
  def delete_payment
    ...
    if paym.paymenttype != "manual"
      flash[:notice] = _('Only_manual_payments_can_be_deleted')
      redirect_to :action => 'list' and return false
    end
...
=end
  ...
  def ouroboros_pay
    ...
    if @enabled == 1
      ...
      @policy = Ouroboros::Payment.format_policy(@ob_max_amount, @retry_count, @completition, @completition_over)
      @amount = Ouroboros::Payment.format_amount(params[:amount], @ob_min_amount, @ob_max_amount)
