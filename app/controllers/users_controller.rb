class UsersController < ApplicationController
  ...
  include SqlExport
  #before_filter :check_localization, :authorize_admin
  ...
  verify :method => :post, :only => [ :destroy, :create, :update, :update_personal_details ],
    :redirect_to => { :action => :list },
    :add_flash => { :notice => _('Dont_be_so_smart'),
    :params => {:dont_be_so_smart => true}}

  User.content_columns.each do |column|
    in_place_edit_for :user, column.name, :empty_text => '...'
  end
  ...
  def list
    ...
    @a_number = []
    for user in @users
      @a_number[user.id] = user.accounting_number
    end
