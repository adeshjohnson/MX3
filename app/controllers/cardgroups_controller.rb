...
class CardgroupsController < ApplicationController
  ...
  def list
    ...
    @cardgroups = Cardgroup.find(:all, :select => "cardgroups.*, COUNT(*) AS card_count", :joins => "LEFT JOIN cards ON cards.cardgroup_id = cardgroups.id", :include => [:tax], :conditions=>["cardgroups.owner_id = ? AND (cards.hidden =0 or cards.id is null)", user_id], :group => 'cardgroups.id')
  end
