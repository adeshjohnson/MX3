class ProvidersController < ApplicationController
  ...
  verify :method => :post, :only => [ :destroy, :create, :update, :delete ],
    :redirect_to => {:controller=>:callc, :action => :main },
    :add_flash => { :notice => _('Dont_be_so_smart'),
    :params => {:dont_be_so_smart => true}}

  Providerrule.content_columns.each do |column|
    in_place_edit_for :providerrule, column.name
  end
  ...
  def create
    ...
    if @provider.save
      ...
      if not dev.save
        @provider.destroy
        flash[:notice] = _('Provider_was_not_created')
        redirect_to :action => 'new' and return false
      end
  ...
  def update
    ...
    unless @provider.valid?
      flash[:notice] = _('Providers_was_not_saved')
      @provider.errors.each { |e|
        flash[:notice] += " " +e[1]
      }
    ...
    if @provider.save
      ...
      if @provider.tech == "SIP" or @provider.tech == "IAX2"
        ...
        #else
        exceptions = @provider.prune_device_in_servers(@device.username)
        #end
