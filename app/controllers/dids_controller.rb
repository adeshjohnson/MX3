class DidsController < ApplicationController
  ...
  Didrate.content_columns.each do |column|
    in_place_edit_for :didrate, column.name
  end
...
=end
  def list
    ...
    if @search_user and @search_user.to_i.to_s == @search_user
      @devices = Device.find(:all, :select => "id, device_type, extension, name", :conditions => ["devices.user_id = ?", @search_user.to_i], :order => "devices.name ASC")
    else
...
=end
  ...
  def did_rates
    #    MorLog.my_debug(@allow_read)
    #    MorLog.my_debug(@allow_edit)
...
#      @allow_read = true
    ...
    @did_prov_rates = @did.did_prov_rates
    @did_incoming_rates = @did.did_incoming_rates
    @did_owner_rates = @did.did_owner_rates
...
ORDER BY dids.did ASC"
  ...
  def confirm_did_action
    ...
    when 1 then opts[:action] = :dids_interval_edit
    when 2 then opts[:action] = :dids_interval_delete
    when 3 then opts[:action] = :dids_interval_rates
    when 4 then opts[:action] = :dids_interval_trunk
    when 5 then opts[:action] = :dids_interval_add_to_user
    when 6 then opts[:action] = :dids_interval_assign_dialplan
    else
