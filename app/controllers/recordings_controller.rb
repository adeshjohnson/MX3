class RecordingsController < ApplicationController
  ...
  def show
    ...
    sql = "SELECT * FROM recordings WHERE SUBSTRING(datetime,1,10) BETWEEN '#{session_from_date}' AND '#{session_till_date}' AND (src_device_id = '#{@device.id}' OR dst_device_id = '#{@device.id}') ORDER BY datetime DESC LIMIT #{@from}, #{@to}"

    my_debug sql
    
    @recs = Recording.find_by_sql(sql)
    @total_pages = Recording.count(:all, :conditions=>"SUBSTRING(datetime,1,10) BETWEEN '#{session_from_date}' AND '#{session_till_date}' AND (src_device_id = '#{@device.id}' OR dst_device_id = '#{@device.id}')")
...
=end 
  ...
  def bulk_management
    ...
    if params[:rec_action].to_i == 0
      @devices = Device.find(:all, :include=>[:user], :conditions=>['users.owner_id = ?', correct_owner_id])
    else
      cond = 'id = -1'
