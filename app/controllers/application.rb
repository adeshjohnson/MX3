# Little Enchancment for all Enumerable items
module Enumerable
  def dups
...
#    if (type == :decimal or type == :float or type == :fixumn) && value.is_a?(String)
...
class ApplicationController < ActionController::Base
  ...
  helper_method :admin?, :reseller?, :user?, :accountant?, :reseller_pro_active?, :show_recordings?
  before_filter :set_charset, :adjust_json_formatting
  before_filter :set_current_user
  # before_filter :set_timezone

  filter_parameter_logging :psw
  ...
  def set_current_user
    ...
    User.sistem_time_ofset = session[:time_zone_ofset].to_i
  end
  ...
  def check_localization
    ...
    if current_user
      session[:year_from] = current_user.user_time(Time.now).year if session[:year_from].to_i == 0
      session[:month_from] = current_user.user_time(Time.now).month if session[:month_from].to_i == 0
      session[:day_from] = current_user.user_time(Time.now).day if session[:day_from].to_i == 0

      session[:year_till] = current_user.user_time(Time.now).year if session[:year_till].to_i == 0
      session[:month_till] = current_user.user_time(Time.now).month if session[:month_till].to_i == 0
      session[:day_till] = current_user.user_time(Time.now).day if session[:day_till].to_i == 0
    else
      session[:year_from] = Time.now.year if session[:year_from].to_i == 0
      session[:month_from] = Time.now.month if session[:month_from].to_i == 0
      session[:day_from] = Time.now.day if session[:day_from].to_i == 0

      session[:year_till] = Time.now.year if session[:year_till].to_i == 0
      session[:month_till] = Time.now.month if session[:month_till].to_i == 0
      session[:day_till] = Time.now.day if session[:day_till].to_i == 0
    end
  end
  ...
  def configure_extensions(device_id, options = {})
    ...
    @user = @device.user if @device.user_id.to_i > -1
  ...
  def renew_session(user)
    ...
    if Confline.find(:first, :conditions=>'name = "Sistem_time_zone_ofset"')
      session[:time_zone_ofset] = Confline.get_value('Sistem_time_zone_ofset').to_i
    else
      sql = "select timediff(now(),convert_tz(now(),@@session.time_zone,'+00:00'));"
      z = ActiveRecord::Base.connection.select_value(sql)
      t = z.to_i
      Confline.set_value('Sistem_time_zone_ofset', t.to_i, 0)
      session[:time_zone_ofset] = Confline.get_value('Sistem_time_zone_ofset').to_i
    end
  ...
  def direction_by_dst(dst)
    sql = "SELECT directions.name FROM directions, destinations WHERE directions.code = destinations.direction_code AND destinations.prefix=SUBSTRING('#{dst}', 1, LENGTH(destinations.prefix)) ORDER BY LENGTH(destinations.prefix) DESC LIMIT 1"
    res = ActiveRecord::Base.connection.select_value(sql)
    return res if res
    return _('Unknown')
  end
      ...
      if exception.to_s.scan(/no route found to match \"\/images\//).size > 0
        if exception.to_s.scan(/no route found to match \"\/images\/flags\//).size > 0
          ...
          if simple_file_name?(country)
            MorLog.my_debug(" >> cp #{RAILS_ROOT}/public/images/flags/empty.jpg #{RAILS_ROOT}/public/images/flags/#{country}", true)
            MorLog.my_debug(`cp #{RAILS_ROOT}/public/images/flags/empty.jpg #{RAILS_ROOT}/public/images/flags/#{country}`)
          end
        end
      end
  ...
  def rescue_action_in_public(exception)
    ...
    begin
      ...
      if important_exception(exception)
        ...
        if exception_class.include?("SystemExit")
          flash_help_link = "http://wiki.kolmisoft.com/index.php/GUI_Error_-_SystemExit"
          Action.new(:user_id => session[:user_id].to_i, :date => Time.now.to_s(:db), :action => "error", :data => 'Asterik_server_connection_error', :data2 => exception.message).save
        end
        ...
        if !flash_help_link.blank?
          ...
          flash[:notice] += "<a id='exception_info_link' href='#{flash_help_link}' target='_blank'><img alt='Help' src='#{Web_Dir}/images/icons/help.png' title='#{_('Help')}' /></a>"
        else
...
=end
  ...
  def can_see_finances?(can_see = nil)
    ...
    logger.fatal session[:acc_see_financial_data].inspect
    can_see
  end
  ...
  def redirect_back_or_default(default = 'callc/main')
    session[:return_to] ? redirect_to_url(session[:return_to]) : redirect_to(Web_Dir + default)
  ...
  def monitoring_enabled_for(user)
    monitorings_addon_active? and user and ["admin", "accountant"].include?(user.usertype)
  end
