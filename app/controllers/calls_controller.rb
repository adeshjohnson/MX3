...
=end
  ...
  def aggregate
    ...
    cond << "calls.user_id IN (SELECT id FROM users WHERE id = #{q(@options[:originator].to_i)} OR owner_id = #{q(@options[:originator].to_i)})"  if @options[:originator] != "any"
    ...
    cond << "calls.id NOT IN (SELECT calls.id FROM calls INNER JOIN providers ON (providers.id = calls.provider_id) WHERE (#{(cond+['billsec = 0 AND disposition = "ANSWERED"']).join(" AND ")} ))"
