  ...
  def my_rescue_action_in_public(exception)
    ...
    begin
      ...
      if important_exception(exception)
        ...
        if exception_class.include?('RuntimeError') and (exception.message.include?('Connection timed out') or exception.message.include?('Connection reset by peer') or exception.message.include?('Network is unreachable') or exception.message.include?('exit'))
