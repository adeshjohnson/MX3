...
=end
  ...
  def hide_destination_end(user)
    if user.attributes["hide_destination_end"] == -1
      if session["owner_hide_dst_end_#{user.owner_id}".to_sym].blank?
        value = Confline.get_value("Hide_Destination_End", user.owner_id).to_i
        session["owner_hide_dst_end_#{user.owner_id}".to_sym] = value
      else
        value = session["owner_hide_dst_end_#{user.owner_id}".to_sym].to_i
      end
    else
      value = user.attributes["hide_destination_end"]
    end
