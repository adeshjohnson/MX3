class DevicesController < ApplicationController
  ...
  Pdffaxemail.content_columns.each do |column|
    in_place_edit_for :pdffaxemail, column.name, :empty_text => '...'
  end
  ...
  def device_edit
    ...
    #------- VM ----------
    if not @device.voicemail_box
      user = @device.user
      email = ""
      email = user.address.email if user.address
      @device.create_vm(@device.extension, random_digit_password(4), user.first_name + " " + user.last_name, email)
    end
  ...
  def device_update
    ...
    if params[:ip_authentication].to_i == 1 and params[:dynamic_check].to_i == 0 and params[:host].blank?
      flash[:notice] = _("Must_specify_host_if_ip_authentication_and_not_dynamic")
      redirect_to :action => :device_edit, :id => @device.id and return false
    end
    ...
    if params[:device][:extension] and Device.find(:first, :conditions => ["id != ? and extension = ?",@device.id, params[:device][:extension] ])
      ...
      params[:device] = params[:device].reject{|key, value| ['extension'].include?(key.to_s)} if current_user.usertype == 'reseller'
      ...
      @device.update_codecs_with_priority(params[:codec]) if params[:codec]
      #============= PERMITS ===================
      if params[:mask1]
      ...
      @device.port = "4569" if (not params[:port] or params[:port].length < 3) and @device.device_type == "IAX2"
      @device.port = "5060" if (not params[:port] or params[:port].length < 3) and @device.device_type == "SIP"
  ...
  def device_forward
    ...
    if params[:group]
      ...
      @devices = Device.find(:all, :order => "extension ASC")
    end
  ...
  def forwards
    ...
    @page_icon = "forward.png"
    @devices = Device.find(:all, :conditions => "user_id != 0", :order => "name ASC")
  end
  ...
  def cli_user_devices
    ...
    #    else
    @devices = Device.find(:all , :conditions => ["user_id = ?", @num]) if @num.to_i != -1
    #    end
...
my_debug params.to_yaml
          ...
          if  params[:cf_data].to_i == 7
            cf.data  = params[:username]
            cf.data2 = "sip_uri"
            cf.data3 = params[:port]
            cf.data4 = params[:host]
            cf.save
          end
=end
    ...
    if err.to_i == 0
      ...
      if session[:usertype] != "admin" and session[:usertype] != "accountant"
        if session[:usertype] == "user" and session[:manager_in_groups].size == 0
          ...
          @devices = Device.find(:all, :include => [:user], :conditions => ["(users.owner_id = ? OR users.id = ? ) AND devices.accountcode > 0", session[:user_id], session[:user_id]], :order => "name ASC")
          @fax_devices = Device.find(:all, :include => [:user], :conditions => ["(users.owner_id = ? OR users.id = ? ) AND devices.device_type = 'FAX'", session[:user_id], session[:user_id]], :order => "name ASC")
          for group in session[:manager_in_groups]
            for user in group.users
              for device in user.devices
        ...
        #admin
        @devices = Device.find(:all, :conditions => "user_id != -1 AND accountcode > 0", :order => "name ASC")
        @fax_devices = Device.find(:all, :conditions => "user_id != -1 AND device_type = 'FAX'", :order => "name ASC")
      end
  ...
  #            @device = Device.find(params[:id])
  #            @user = User.find(session[:user_id])
  #            redirect_to :controller => "callc", :action => 'main' if @device.user_id != @user.id
  #          else
  #            #group manager
  #            @device = Device.find(params[:id])
  #            @user = @device.user
  ...
  ##        @device.timeout = value
  ##        @device.save
  ##        flash[:notice] = _('Settings_saved')
  ##        redirect_to :action => 'callflow' , :id => @device.id
  ##      end
  #
  #    end
  ...
  def user_devices
    @page_title = _('Devices')
    @page_icon = "device.png"
    @devices = Device.find(:all,:include => [:user, :provider], :conditions => ["devices.user_id = ?", session[:user_id]], :order => "devices.name")
  end
...
=end
  ...
  def default_device_update
    ...
    Confline.set_value("Default_device_port", "4569", session[:user_id])  if (not params[:port] or params[:port].length < 3) and @device_type == "IAX2"
    Confline.set_value("Default_device_port", "5060", session[:user_id]) if (not params[:port] or params[:port].length < 3) and @device_type == "SIP"
...
=end
  def get_user_devices
    ...
    if @user == "all"
      @devices = Device.find(:all, :select=> "devices.*", :joins=>"LEFT JOIN users ON (users.id = devices.user_id)" , :conditions => ["users.owner_id = ? AND device_type != 'FAX'", owner_id])
    else
      @devices = Device.find(:all, :select=> "devices.*", :joins=>"LEFT JOIN users ON (users.id = devices.user_id)" , :conditions => ["users.owner_id = ? AND device_type != 'FAX' AND user_id = ? ", owner_id, @user])
    end
    render :layout => false
  end
