...
=begin
Copied from rails source. This does not work directly. Why? Reason - unknown.
=end
...
=end
  ...
  def nice_credit(user)
    if user.credit and user.postpaid == 1
      credit = user.credit == -1 ? _('Unlimited') : user.credit
    else
      credit = 0
    end
    credit
  ...
  def nice_user_tooltip(user)
    tooltip('User details', "<b>Tariff:</b> #{user.tariff.name if user.tariff} <br> <b>LCR:</b> #{user.lcr.name if user.lcr}<br> <b>Credit:</b> #{nice_credit(user)}")
  end
